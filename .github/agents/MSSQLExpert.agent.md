...
SQL Server SPs serve two distinct purposes:
...
Called by `SqlChangeTrackerClient` to detect changes since a transaction ID:

- **Input**: Single parameter — transaction ID (type from `QueryParameterType`, usually `bigint`)
- **Output columns**: Must include:
  - Primary key column (name from `ResultPrimaryKeyColumnName`)
  - Transaction ID column (name from `ResultTransactionIdColumnName`)
- **Behavior**: Return all rows changed since the given transaction ID
- **Requirement**: Transaction IDs must be strictly increasing — returning the same ID causes a recursion guard exception in `SqlChangeTrackerJob`

### Data Loader SPs

Called by `SqlDataLoaderClient` to load full entity data by keys:

- **Input**: TVP (table-valued parameter) or single parameter with keys
  - TVP: `QueryParameterTypeName` specifies the SQL type name
  - `QueryParameterName` specifies the parameter name
- **Output**: Full entity data for the provided keys
- **Requirement**: Result must include `ResultPrimaryKeyColumnName` for correlation

### SP Design Rules

```sql
-- Every SP must include:
SET NOCOUNT ON;
...
EXEC dbo.MyStoredProcedure @param;

-- Use TRY/CATCH for error handling in complex SPs
BEGIN TRY
    -- logic
END TRY
BEGIN CATCH
...
## SqlClient Execution Pattern

The C# `SqlClient.cs` wraps all SP calls with:

- `CommandType.StoredProcedure` — exclusively stored procedures, no inline SQL
- `CommandTimeout` = 10 minutes (`TimeSpan.FromMinutes(10).Seconds`)
- Polly `WaitAndRetry(3, attempt => TimeSpan.FromSeconds(5))` for transient `SqlException`
- Streaming via `IAsyncEnumerable<SqlTable>` with optional batching
- `SqlDataReader` with field-by-field row reading (`GetValue`, `GetName`)
- `DBNull.Value` → `null` conversion

Implications for SP design:
- SPs may be called with long timeouts — avoid unnecessary blocking
- Transient errors are retried 3 times — ensure SPs are idempotent or safe to retry
- Results stream through batches — order of results matters for change tracker transaction ID tracking
...
|---|---|
| `SqlChangeTrackerConfiguration` | `StoredProcedure`, `QueryParameterName`, `QueryParameterType`, `InitialTransactionId`, `ResultPrimaryKeyColumnName`, `ResultTransactionIdColumnName`, `CronSchedule`, `ConnectionString` |
| `SqlLoaderConfiguration` | `StoredProcedure`, `QueryParameterName`, `QueryParameterType`, `QueryParameterTypeName` (TVP), `ResultPrimaryKeyColumnName`, `ConnectionString` |

`CronSchedule` default: `"0 {0} * ? * * *"` — placeholder `{0}` is replaced with a minute offset.

## Testing with Squadron

Integration tests use Squadron `SqlServerResource<SqlServerOptions>` for real SQL Server instances:
